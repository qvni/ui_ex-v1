...
B3Spell_AutoPause                     = Infinity_GetINIValue('Bubbs Spell Menu Extended', 'Auto-Pause',                       0)
B3Spell_DarkenBackground              = Infinity_GetINIValue('Bubbs Spell Menu Extended', 'Darken Background',                0)
B3Spell_Modal                         = Infinity_GetINIValue('Bubbs Spell Menu Extended', 'Modal',                            0)
...
B3Spell_SlotsGapX         = 0
B3Spell_MinY              = 70  --[[1 + 5 + 27 + 5]]--
B3Spell_SlotsGapY         = 0
B3Spell_SlotsGapYFlowover = 0

B3Spell_SidebarWidth = 40
B3Spell_Menu_OptimizeSlotSize_AlignmentX = 208 + 200
...
function B3Spell_LaunchSpellMenu(mode)
	...
	B3Spell_Mode = mode
	B3Spell_OpenerID = EEex_GetActorIDSelected()
...
function B3Spell_InitializeSlots()
	...
	if B3Spell_AlignCenter then
		...
		Infinity_SetArea('B3Spell_Menu_FilterSlotsMage',   B3Spell_SidebarWidth -54,       nil, nil, nil)
		Infinity_SetArea('B3Spell_Menu_FilterSlotsAll',    B3Spell_SidebarWidth -54 --[[+ 52]],  nil, nil, nil)
		Infinity_SetArea('B3Spell_Menu_FilterSlotsCleric', B3Spell_SidebarWidth -54 --[[+ 104]], nil, nil, nil)
...
function B3Spell_Menu_Tick()
    ...
    for i = 0, 5, 1 do
        local id = EEex_GetActorIDPortrait(i)
        if id ~= -1 then
            portraitIDs[id] = true
    ...
    local numSelectedActors = #selectedActors

    local foundPortraits = 0
    for _, actorID in ipairs(selectedActors) do
        if portraitIDs[actorID] then
            foundPortraits = foundPortraits + 1
        end
    end

    if numPortraits ~= foundPortraits and (numSelectedActors ~= 1 or selectedActors[1] ~= B3Spell_OpenerID) then
        B3Spell_UnselectCurrentButton()
        Infinity_PopMenu("B3Spell_Menu")
        return
    end
	
	if Infinity_IsMenuOnStack("INVENTORY") then
		Infinity_PopMenu("B3Spell_Menu")
		return
	end
	
	if Infinity_IsMenuOnStack("CHARACTER") then
		Infinity_PopMenu("B3Spell_Menu")
		return
	end
	...
	if Infinity_IsMenuOnStack("CHARGEN_PROFICIENCIES") then
		Infinity_PopMenu("B3Spell_Menu")
		return
	end	
	
	--[[local selectedActors = EEex_GetAllActorIDSelected()
	if #selectedActors ~= 1 or selectedActors[1] ~= B3Spell_OpenerID then
		B3Spell_UnselectCurrentButton()
		Infinity_PopMenu("B3Spell_Menu")
		return
	end]]--
	...
	if B3Spell_SearchEdit ~= B3Spell_OldSearchEdit then --[[testing]]--
